...
**Pre-requisite:** [Search Element in Rotated Sorted Array I](https://takeuforward.org/data-structure/search-element-in-a-rotated-sorted-array/) **&** [Binary Search algorithm](https://takeuforward.org/data-structure/binary-search-explained/)
...
![](https://static.takeuforward.org/wp/uploads/2023/06/Screenshot-2023-06-21-004311.png)
...
One straightforward approach we can consider is using the [linear search algorithm](https://takeuforward.org/data-structure/linear-search-in-c/). Using this method, we will traverse the array to check if the target is present in the array. If it is found we will simply return True and otherwise, we will return False.
...
Like the [previous problem](https://takeuforward.org/data-structure/search-element-in-a-rotated-sorted-array/), we will use the [Binary Search algorithm](https://takeuforward.org/data-structure/binary-search-explained/) to solve this problem.
...
In the [previous problem](https://takeuforward.org/data-structure/search-element-in-a-rotated-sorted-array/), in order to efficiently search for the target value, we followed a simple two-step process. 
...
This check was effective in the previous problem, where there were no duplicate numbers. However, in the current problem, the array may contain duplicates. **_Consequently, the previous approach will not work when arr[low] = arr[mid] = arr[high]._**
...
![](https://static.takeuforward.org/wp/uploads/2023/06/Screenshot-2023-06-21-012150.png)

**Note**: _As long as this condition is met, we will skip the steps of determining the sorted half and eliminating one of the halves based on the target's location. Instead, we will solely focus on eliminating arr[low] and arr[high]._
...
4. **Check if arr[low] = arr[mid] = arr[high]:** If this condition is satisfied, we will just increment the low pointer and decrement the high pointer by one step. We will not perform the later steps until this condition is no longer satisfied. So, we will **_continue to the next iteration from this step_**.
...
**Dry-run:** _Please refer to the_ [_video_](https://youtu.be/w2G2W8l__pc) _for it._
