...
**Pre-requisites:** [Find minimum in Rotated Sorted Array](https://takeuforward.org/data-structure/minimum-in-rotated-sorted-array/),  [Search in Rotated Sorted Array II](https://takeuforward.org/strivers-a2z-dsa-course/strivers-a2z-dsa-course-sheet-2/) & [Binary Search algorithm](https://takeuforward.org/data-structure/binary-search-explained/)
...
![](https://static.takeuforward.org/wp/uploads/2023/06/Screenshot-2023-06-27-231533.png)

**Observation:** 

- We can easily observe that **_the number of rotations in an array is equal to the index(0-based index) of its minimum element._**
- _So,_ **_in order to solve this problem, we have to find the index of the minimum element._**
...
3. **If arr[low] <= arr[high]:** In this case, the array from index low to high is completely sorted. Therefore, we can select the minimum element, arr[low].  
    Now, **_if arr[low] < ans, we will update ‘ans’ with the value arr[low] and ‘index’ with the corresponding index low._**  
...
4. Identify the sorted half, and after picking the leftmost element, eliminate that half.
    ...
    2. **Otherwise, if the right half is sorted:**  This condition ensures that the right half is sorted. So, we will pick the leftmost element i.e. **arr[mid]**.  
        Now, **_if arr[mid] < ans, we will update ‘ans’ with the value arr[mid] and ‘index’ with the corresponding index mid._**  
...
**Dry-run:** _Please refer to the_ [_video_](https://youtu.be/jtSiWTPLwd0) _for a detailed explanation._
